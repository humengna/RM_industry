...
> `config.STRATEGY_MODE = 'original'`（默认）时本节内容不生效，
> 策略买动量第 1 名；改成 `'risk'` 才启用。
...
通过日线体检的候选会再过一遍分钟线风控（`momentum_timing/intraday.py`）：
用前几个完整交易日的日内走势和成交金额分布，识别炸板、尾盘跳水、收在日内低位、
跌破 VWAP、高位派发、盘中触及跌停、天量滞涨。分钟线按需拉取，
日线就被否决的候选不会去取。
...
- 新增日线风控、分钟线风控与大盘风控（见 [risk.md](risk.md)），默认开启，
  会改变选股结果；想对照原始行为，把 `RISK_ENABLED`、`INTRADAY_ENABLED`、
  `MARKET_FILTER_ENABLED` 设为 `False` 即可。
